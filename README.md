...
prev_size와 list_head는 포함되지 않습니다.

즉, 이 두 필드는 할당 청크의 실제 크기 계산에서 제외됩니다.

반대로 해제된 청크는 원래 사용자 데이터로 사용되던 영역을 재활용하여   
prev_size와 list_head를 저장합니다.

이 방식은 청크에 포함되는 고정 메타데이터 크기를 줄여   
힙 영역의 메모리를 효율적으로 사용할 수 있도록 합니다.
...
자식이 처음 스케줄링될 때 부모와 동일한 문맥으로 복귀하도록 설정합니다.
...
- pid > 0  
  특정 PID를 가진 자식 프로세스의 종료를 감지
- pid == 0  
  호출한 프로세스와 동일한 프로세스 그룹 내의 자식 종료를 감지
- pid < -1  
  -pid를 PGID로 하여 해당 프로세스 그룹 내의 자식 종료를 감지
- pid == -1  
  임의의 자식 프로세스 종료를 감지  
...
차단형 시스템 콜로 인해 프로세스가 대기 상태로 전환될 때 발생합니다.   
